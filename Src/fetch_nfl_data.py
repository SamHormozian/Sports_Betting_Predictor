...
os.makedirs("data/csv/nfl", exist_ok=True)
...
def fetch_nfl_teams():
    """
    Fetch NFL teams and save to both JSON and CSV files.
    """
    data = fetch_with_retry(NFL_TEAMS_URL)
    if data:
        # Save raw JSON
        raw_file_path = "data/raw/nfl/teams.json"
        with open(raw_file_path, "w") as f:
            json.dump(data, f, indent=4)
        print(f"NFL team data saved to {raw_file_path}")
        ...
        csv_file_path = "data/csv/nfl/teams.csv"
...
def fetch_nfl_scoreboard():
    """
    Fetch NFL scoreboard data and save to both JSON and CSV files.
    """
    data = fetch_with_retry(NFL_SCOREBOARD_URL)
    if data:
        # Save raw JSON
        raw_file_path = "data/raw/nfl/scoreboard.json"
        with open(raw_file_path, "w") as f:
            json.dump(data, f, indent=4)
        print(f"NFL scoreboard data saved to {raw_file_path}")

        # Convert scoreboard data to CSV
        events = data.get("events", [])
        csv_file_path = "data/csv/nfl/scoreboard.csv"
